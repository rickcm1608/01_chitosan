...
Production run: **50 ns** (25,000,000 steps × 2 fs, 1000 frames saved every 50 ps)  
...
│       ├── rdf_blocks/    # Per-block CHI–WAT RDF (10 blocks × 5 ns)
...
└── oil_water/
    ...
    └── data/
        ...
        ├── rdf_chi_wat/   # Per-block CHI–WAT RDF (25 blocks × 2 ns)
...
| `02_water_coordination.py` | Water O atoms within 3.5 Å of CHI per frame | `water_coordination.csv` |
| `03_hbonds_chi_water.py` | H-bond count CHI↔WAT per frame (distance ≤ 3.5 Å) | `hbonds_chi_wat.csv` |
...
- **Rg (water-only):** ~35.5 Å → chitosan adopts a compact conformation in pure water
- **Rg (oil-water):** ~41.9 Å → chitosan adopts a more extended conformation upon interaction with oil molecules
- **RDF CHI–WAT:** first coordination shell at ~3.7 Å; reduced in oil-water system
- **RDF CHI–NAP/PAR:** direct adsorption contacts confirmed at short range

Data used in paper figures: `rdf_averaged/chi_wat_rdf_avg.txt` (water_only) and `rdf_chi_wat/` blocks 21–25 average (oil_water); `radius_of_gyr